...
class Team(EffectiveDateMixin, SoftRetirementMixin, TenantEntity):
    """An optional execution unit that may span organizational boundaries."""
    ...
    managed_store_id: Mapped[UUID | None] = mapped_column(UUID_TYPE, nullable=True)
    department_id: Mapped[UUID | None] = mapped_column(UUID_TYPE, nullable=True)
    leader_employee_id: Mapped[UUID | None] = mapped_column(UUID_TYPE, nullable=True)
    ...
    __table_args__ = (
        ...
        Index("ix_teams_v5_enterprise_status", "enterprise_id", "status"),
        Index("ix_teams_v5_enterprise_type", "enterprise_id", "team_type"),
        Index(
            "ix_teams_v5_organizational_group",
            "enterprise_id",
            "organizational_group_id",
        ),
        Index("ix_teams_v5_managed_store", "enterprise_id", "managed_store_id"),
        Index("ix_teams_v5_department", "enterprise_id", "department_id"),
        Index("ix_teams_v5_leader", "enterprise_id", "leader_employee_id"),
    )
    ...
    def validate_code(self, _key: str, value: str) -> str:
        ...
        if not _CODE_PATTERN.fullmatch(normalized):
            raise ValueError(
                "Team code must contain 2-40 uppercase letters, digits, underscores, or hyphens."
            )
        return normalized
    ...
    def __repr__(self) -> str:
        return (
            f"<Team team_id={self.id} enterprise_id={self.enterprise_id} "
            f"code={self.code!r} name={self.name!r} team_type={self.team_type.value!r}>"
        )
